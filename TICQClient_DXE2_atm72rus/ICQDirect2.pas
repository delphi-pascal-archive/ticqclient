...
Type
  ...
  TicqDCM = Class(TObject)
    Private
      ...
      fUserID: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
      fResolve: Boolean;
      fOnError: TOnError;
      ...
      procedure InternalOnErrorProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); virtual;
    ...
    Public
      ...
      property ProxyUserID: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read FUserID write FUserID;
  ...
  TicqBaseDC = Class(TObject)
    Protected
      ...
      procedure OnIntError(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); virtual;
    Public
      EventType:Integer;
      MyUIN:LongWord;
  ...
  TicqDCRecvFile = Class(TicqBaseDC)
    ...
    Public
      ...
      TotalBytes:LongWord;
      NickName:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
Function IPToStr(aIP:LongWord):{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Var
  HV:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Begin
  HV := IntToHex(aIP, 8);
  Result := '';
...
Begin
  ...
  fDCL.Free;
//  FreeMemory(fpUser);
  FreeMem(fpUser, SizeOf(TicqDirectUser)); // yegor
End;
...
Begin
  ...
  aTC := Int64(GetTickCount) - Int64(MAXTICKS_TIMEOUT);
...
procedure TicqDCM.InternalOnErrorProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
...
procedure TicqBaseDC.OnIntError(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
  If Assigned(fOnError) Then
    fOnError(Sender, ErrorType, ErrorMsg);
End;
