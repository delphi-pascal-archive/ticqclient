...
type
  ...
  TTranslateFunction = function(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  TICQLangNode = record
    Lang: TICQLangType;
    Translate: TTranslateFunction;
  end;
  function LangEn(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  function LangRu(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  function LangHu(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  function LangNo(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  function LangDe(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
implementation
  ...
  begin
    case ResID of
      ...
      {Runtime errors}
      IMSG_EUINONOTHERCOMPUTER:         Result := 'Соединение прервано сервером по причине использования данного UIN другим ICQ-клиентом'
    else
      {If we cannot translate, return the english version of the string}
      Result := ICQLanguages[LANG_EN].Translate(ResID);
    end;
  end;
  function LangHu(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  // Translated by Mickey (mickey777@mail.datanet.hu)
  begin
    case ResID of
      {ICQClient}
  ...
  function LangNo(ResID: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  // Translated by Dan Michael (danm@start.no)
  begin
    case ResID of
      {ICQClient}
