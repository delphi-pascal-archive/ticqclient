...
type
  ...
  public
    ...
    procedure AddStr(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
    function GetStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    function GetLength: LongWord;
  end;

  TOnSocketThreadException = procedure (Sender: TObject; AExceptionStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) of object;
  ...
  protected
    ...
    FPort: Word;
    FLastError: Word;
    FLastErrMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    Buffer: Pointer;
    BufLen: LongWord;
    function Resolve(const Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Integer;
  ...
  public
    ...
    procedure SendStr(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
  ...
  private
    FDestHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FDestIp: Integer;
    FDestPort: Word;
  ...
  protected
    procedure Execute; override;
  public
    property Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read FDestHost write FDestHost; {Real Host}
    property Port: Word read FDestPort write FDestPort;   {Real Port}

    property ProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read FHost write FHost;
  ...
  TOnError = procedure(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) of object;
  ...
  TOnAdvPktParse = procedure(Sender: TObject; Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Port: Word; Buffer: Pointer; BufLen: LongWord; Incoming: Boolean) of object;
  ...
  TSrvSocket = class(TObject)
    private
      ...
      procedure OnSrvErrProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
  ...
  TMySocket = class(TObject)
    private
      ...
      Procedure SetHost( aHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
      Procedure SetPort( aPort: Word);
      Function  GetHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
      Function  GetPort: Word;
      {$HINTS ON}       // added by eraser 1.6.2003
      Procedure SetProxyType( aProxyType: TProxyType);
      Procedure SetProxyHost( aProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
      Procedure SetProxyPort( aProxyPort: Word);
      Procedure SetProxyAuth( aProxyAuth: Boolean);
      Procedure SetProxyPass( aProxyPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
      Procedure SetProxyUser( aProxyUser: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
      Procedure SetProxyRslv( aProxyRslv: Boolean);
      Function  GetProxyType: TProxyType;
      Function  GetProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
      Function  GetProxyPort: Word;
      Function  GetProxyAuth: Boolean;
      Function  GetProxyPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
      Function  GetProxyUser: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
      Function  GetProxyRslv: Boolean;

      Procedure OnConnectErrorProc(Sender: TObject);
      Procedure OnDisconnectProc(Sender: TObject);
      Procedure OnConnect(Sender: TObject);
      Procedure OnErrorProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
    ...
    public
      ...
      property ProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read GetProxyHost write SetProxyHost;
      property ProxyPort: Word read GetProxyPort write SetProxyPort;
      property ProxyUser: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read GetProxyUser write SetProxyUser;
      property ProxyAuth: Boolean read GetProxyAuth write SetProxyAuth;
      property ProxyPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read GetProxyPass write SetProxyPass;
...
function GetHTTPStatus(List: TStringList): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};

function WSockAddrToIp(Value: LongWord): AnsiString;
function WSAErrorToStr(ErrorNo: Integer): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
procedure ShowMessage(const Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
begin
{$IFDEF VER230}
...
Procedure TMySocket.OnErrorProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
...
Procedure TMySocket.SetHost( aHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
...
Function  TMySocket.GetHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Begin
...
Procedure TMySocket.SetProxyHost( aProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
...
Procedure TMySocket.SetProxyPass( aProxyPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
  FEventSocket.ProxyPass := aProxyPass;
End;

Procedure TMySocket.SetProxyUser( aProxyUser: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
...
Function  TMySocket.GetProxyHost: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Begin
...
Function  TMySocket.GetProxyPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Begin
  Result := FEventSocket.ProxyPass;
End;

Function  TMySocket.GetProxyUser: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
Begin
...
procedure TSrvSocket.OnSrvErrProc(Sender: TObject; ErrorType: TErrorType; ErrorMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
Begin
  If assigned(fOnError) then
...
procedure TNetBuffer.AddStr(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
begin
  AddPacket(@Value[1], Length(Value));
end;

function TNetBuffer.GetStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  p: array[0..CNetPktLen] of Char;
begin
  p[GetPacket(@p)] := #0;
  Result := {$IFDEF VER230}PAnsiChar(@p){$ELSE}PChar(@p){$ENDIF};
end;
...
function TCustomSocket.Resolve(const Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Integer;
var
  he: PHostEnt;
begin
  Result := inet_addr({$IFDEF VER230}PAnsiChar(Host){$ELSE}PChar(Host){$ENDIF});
  if DWord(Result) = DWord(INADDR_NONE) then
  begin
    he := gethostbyname({$IFDEF VER230}PAnsiChar(Host){$ELSE}PChar(Host){$ENDIF});
    if he = nil then Exit;
...
procedure TCustomSocket.SendStr(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
begin
  SendData(pointer(Value), Length(Value));
...
procedure TSOCKS5Socket.OnReceive;
  ...
  begin
    ...
    begin
      buf[3] := 3;
      buf[4] := len - 1;
      Move({$IFDEF VER230}PAnsiChar(Host)^{$ELSE}PChar(Host)^{$ENDIF}, buf[5], len - 1);
    end else
    begin
      buf[3] := 1;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              FSrcBuf[0] := 1;
              FSrcBuf[1] := UserLen;
              Move({$IFDEF VER230}PAnsiChar(ProxyUser)^{$ELSE}PChar(ProxyUser)^{$ENDIF}, Ptr(LongWord(@FSrcBuf) + 2)^, UserLen);
              FSrcBuf[UserLen + 2] := PassLen;
              Move({$IFDEF VER230}PAnsiChar(ProxyPass)^{$ELSE}PChar(ProxyPass)^{$ENDIF}, Ptr(LongWord(@FSrcBuf) + 3 + UserLen)^, UserLen);
...
begin
  if not ProxyReady then begin
    ...
    begin
      ...
      if FLen = 0 then
        if FCurLen > 3 then
          if Copy({$IFDEF VER230}PAnsiChar(@FBuf){$ELSE}PChar(@FBuf){$ENDIF}, FCurLen - 3, 4) = #13#10#13#10 then
          begin
            List := TStringList.Create;
            List.Text := {$IFDEF VER230}PAnsiChar(@FBuf){$ELSE}PChar(@FBuf){$ENDIF};
            ...
            FCurLen := 0;
            {$IFDEF PARSE}
            LogText('http.txt', 'Received (proxy): '#13#10 + {$IFDEF VER230}PAnsiChar(@FBuf){$ELSE}PChar(@FBuf){$ENDIF});
            {$ENDIF}
          end;
...
initialization
  ...
  if WSAStartup(MAKEWORD(1, 1), WSAData) <> 0 then begin
    {$IFDEF DEBUG}
    {$IFDEF VER230}
	MessageBoxA(0, PAnsiChar('Could not start WSA'), 'Error!', MB_ICONERROR);
	{$ELSE}
...
finalization
  if WSACleanUp <> 0 then begin
    {$IFDEF DEBUG}
    {$IFDEF VER230}
	MessageBoxA(0, PAnsiChar('Could not cleanup WSA'), 'Error!', MB_ICONERROR);
	{$ELSE}
