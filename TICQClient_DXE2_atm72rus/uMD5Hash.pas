...
procedure MD5Update(var Context: MD5Context; Input: {$IFDEF VER200}PAnsiChar{$ELSE}PChar{$ENDIF}; Length: longword);
procedure MD5Final(var Context: MD5Context; var Digest: MD5Digest);

function MD5String(M: {$IFDEF VER200}RawByteString{$ELSE}string{$ENDIF}): MD5Digest;
function MD5File(N: {$IFDEF VER200}RawByteString{$ELSE}string{$ENDIF}): MD5Digest;
function MD5Print(D: MD5Digest): {$IFDEF VER200}RawByteString{$ELSE}string{$ENDIF}; 
...
begin
        MD5Init(Context);
        MD5Update(Context, {$IFDEF VER200}PAnsiChar(M){$ELSE}PChar(M){$ENDIF}, length(M));
...
begin
        MD5Init(Context);
        {$IFDEF VER200}
		FileHandle := CreateFileA(PAnsiChar(N), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL or FILE_FLAG_SEQUENTIAL_SCAN, 0);
		{$ELSE}
		FileHandle := CreateFile(PChar(N), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL or FILE_FLAG_SEQUENTIAL_SCAN, 0);
		{$ENDIF}				
