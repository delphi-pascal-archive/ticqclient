...
type
  TOnErrorEvent = procedure(Sender: TObject; Reason: Word; ReasonStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) of object;
  ...
  TOnContact = procedure(Sender: TObject; UIN: LongWord; NickName, FirstName, LastName,
    Email: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Age, Gender: Byte; LastUpdate: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  LastUpdateStamp: LongWord) of object;
  TOnSelfInfo = procedure(Sender: TObject; UIN: LongWord; NickName, FirstName, LastName,
    Email, Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Age, Gender: Byte; LastUpdate: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  LastUpdateStamp: LongWord) of object;
  TOnMessage = procedure(Sender: TObject; UIN: LongWord; Incoming: Boolean; Msg, RecvTime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  RecvTimeStamp: LongWord) of object;
  TOnUrl = procedure(Sender: TObject; UIN: LongWord; Incoming: Boolean; Description, URL, RecvTime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  RecvTimeStamp: LongWord) of object;
  TOnAdvMessage = procedure(Sender: TObject; UIN: LongWord; Incoming: Boolean; PlainText, RichText, UTF8Text, RecvTime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  RecvTimeStamp: LongWord) of object;
  ...
  private
    FIdxFile, FDatFile: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    ...
    function ReadStr(Handle: THandle; Len: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    function ReadLNTS(Handle: THandle): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    ...
    function OpenIdx(const FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
    procedure CloseIdx;
    function OpenDat(const FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
    procedure CloseDat;
  ...
  published
    property DatFile: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}  read FDatFile write FDatFile;
...
implementation
function TimeStamp2Str(Timestamp: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  DelphiTime: Double;
begin
...
function TICQDb.ReadStr(Handle: THandle; Len: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  buf: Pointer;
  read: LongWord;
begin
  ...
  {$IFDEF VER230}
  Result := Copy(PAnsiChar(buf), 0, Len);
  {$ELSE}
...
function TICQDb.ReadLNTS(Handle: THandle): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
...
function TICQDb.OpenIdx(const FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
begin
  Result := False;
  CloseIdx;
  {$IFDEF VER230}
  FHandle := CreateFileA(PAnsiChar(FileName), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, 0, 0);
  {$ELSE}
...
function TICQDb.OpenDat(const FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
begin
  Result := False;
  CloseDat;
  {$IFDEF VER230}
  FDHandle := CreateFileA(PAnsiChar(FileName), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_EXISTING, 0, 0);
  {$ELSE}
...
function Read6bh: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
...
var
  FNickName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FFirstName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FLastName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FEmail: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FLastUpdate: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FAge, FGender: Byte;
  FUIN: LongWord;
  FMsg, FMsg2, FMsg3: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FFlag: LongWord;
  FSeparator: Word;
  FSubType: Word;
  FTStamp: LongWord;

  FPassword: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FCryptIV: LongWord;

procedure ReadProperty;
var
  Len: Word;
  AName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
var
  ...
  Num: LongWord;
  i: LongWord;
  FURL, FDesc: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
  if FDHandle = INVALID_HANDLE_VALUE then Exit;
...
var
  ...
  FAge, FGender: Byte;
  FUIN: LongWord;
  FMsg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FPassword: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};


function GetModuleName(Ofs: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
type
...
function ReadASCIIZ: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
...
var
  FName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  __pos: LongWord;
begin
...
var
  ...
  FModName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
...
var
  FDbEvent: TDbEvent;
  FDesc: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  FURL: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
  if not Seek(FDHandle, Ofs) then Exit;
  while True do begin
...
procedure Register;
begin
  RegisterComponents('ICQClient', [TICQDb]);
end;
