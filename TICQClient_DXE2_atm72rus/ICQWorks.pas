...
const
  ...
  NewXStatus: array[0..36] of record
    dopstr : T_NewXStatus;
    dopxstr : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  end = (
...
type
  ...
  TFTRequestRec = record
    ...
    UIN: LongWord;
    Description: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FileSize: LongWord;
    Seq: Word;
    Port: Word;
  end;
  ...
  TSendFileRec = Record
    UIN:LongWord;
    Nick:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    Seq:Word;
    Files:TStrings;
    FilesCurrent:Integer;
    FilesCount:LongWord;
    FilePath:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FileName:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FileDescription:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
    FileSize:LongWord;
    TotalSize:LongWord;
    Port:Word;
    Speed:Byte;
  ...
  TOnFTInit = procedure(Sender: TObject; UIN: LongWord; FileCount, TotalBytes, Speed: LongWord; NickName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) of object;
  TOnFTStart = procedure(Sender: TObject; StartRec: TFTStartRec; FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  FileSize, Speed: LongWord) of object;
...
function IntToStr(Value: Int64): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function StrToInt(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): LongWord;
function IntToHex(Int: Int64; IntSize: Byte): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function HexToInt(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): LongWord;
procedure ICQEncryptPass(SrcBuf: Pointer; BufLen: LongWord);
procedure ICQEncryptPassStr(var Pass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
function Swap16(Value: Word): Word;
function Swap32(Value: LongWord): LongWord;
function iif(ACondition: Boolean; AIfTrue, AIfFalse: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
procedure PktStr(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure PktLStr(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); overload;
procedure PktLStr(Pkt: PRawPkt; S: LongWord); overload;
procedure PktWStr(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure PktDWStr(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure PktLNTS(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure PktLLNTS(Pkt: PRawPkt; const S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure PktTLV(Pkt: PRawPkt; T, L: Word; V: LongWord); overload;
procedure PktTLV(Pkt: PRawPkt; T: Word; const V: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); overload;
procedure PktTLV(Pkt: PRawPkt; T, L: Word; V: Pointer); overload;
procedure PktLTLV(Pkt: PRawPkt; T: Word; const V: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); overload;
procedure PktLTLV(Pkt: PRawPkt; T,L: Word; V: LongWord); overload;
procedure PktLBTLV(Pkt: PRawPkt; T: Word; V: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  B: Byte); overload;

{-- Extracting data --}
function GetInt(Pkt: PRawPkt; IntSize: Byte): LongWord;
function GetLInt(Pkt: PRawPkt; IntSize: Byte): LongWord;
function GetStr(Pkt: PRawPkt; StrLen: Integer): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetTLVStr(Pkt: PRawPkt; var T: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetTLVInt(Pkt: PRawPkt; var T: Word): LongWord;
procedure GetSnac(Pkt: PRawPkt; var Snac: TSnacHdr);
function GetLStr(Pkt: PRawPkt): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetWStr(Pkt: PRawPkt): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetDWStr(Pkt: PRawPkt): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetLNTS(Pkt: PRawPkt): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};


{High-level functions}
function StrToLanguageI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
function StrToCountryI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
function StrToInterestI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
function StrToOccupationI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
function StrToPastI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
function StrToOrganizationI(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Word;
procedure ParseContacts(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var List: TStringList);
function MakeContactsStr(Contacts: TStringList): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};

{Packet creation functions}

procedure CreateCLI_SSL_ACTIVATE(Pkt: PRawPkt; var Seq, Seq2: Word); {CLI_ROSTERACK}

procedure CreateCLI_IDENT(Pkt: PRawPkt; UIN: LongWord; Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Secure: Boolean; var Seq: Word);
procedure CreateCLI_COOKIE(Pkt: PRawPkt; const Cookie: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
...
procedure CreateCLI_SETNEWXSTATUS(Pkt: PRawPkt; XStat: T_NewXStatus; XStatStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
procedure CreateCLI_SETNEWXSTATUS_icqmood(Pkt: PRawPkt; MoodNum: Byte; XStatStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
...
procedure CreateCLI_REQ_AVATAR(Pkt: PRawPkt; UIN: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var FSeq: Word; AvatarId: Word; AvatarFlags: Byte; AvatarHash: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure CreateCLI_TOICQSRV(Pkt: PRawPkt; UIN: LongWord; Command: Word; Data: Pointer; DataLen: LongWord; var Seq, Seq2: Word);
procedure CreateCLI_ADDCONTACT(Pkt: PRawPkt; UIN: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
...
procedure CreateCLI_SENDMSG(Pkt: PRawPkt; ITime, IRandom, UIN: LongWord; const Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
procedure CreateCLI_SENDURL(Pkt: PRawPkt; ITime, IRandom, MyUIN, UIN: LongWord; const URL, Description: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
procedure CreateCLI_AUTHORIZE(Pkt: PRawPkt; UIN: LongWord; Auth: Byte; Reason: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
procedure CreateCLI_METAREQINFO(Pkt: PRawPkt; UIN, dUIN: LongWord; var Seq, Seq2: Word);
procedure CreateCLI_SEARCHBYMAIL(Pkt: PRawPkt; UIN: LongWord; const Email: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq, Seq2: Word);
procedure CreateCLI_SEARCHBYUIN(Pkt: PRawPkt; UIN: LongWord; DUIN: LongWord; var Seq, Seq2: Word);
procedure CreateCLI_SEARCHBYNAME(Pkt: PRawPkt; UIN: LongWord; const FirstName, LastName, NickName, Email: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq, Seq2: Word);
procedure CreateCLI_SEARCHRANDOM(Pkt: PRawPkt; UIN: LongWord; Group: Word; var Seq, Seq2: Word);
procedure CreateCLI_SEARCHWP(Pkt: PRawPkt; UIN: LongWord; First, Last, Nick, Email: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  MinAge, MaxAge: Word; Gender: Byte; Language: Byte; City: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Country: Word; Company, Department, Position: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Occupation: Byte; Organization: Word; OrganKeyWords: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Affiliation: Word; AffiKeyWords, KeyWord: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Online: Byte; var Seq, Seq2: Word);

procedure CreateCLI_SETSELFINFO(Pkt: PRawPkt; UIN: LongWord;
  NickName, FirstName, LastName, Email, City, State, Phone, Fax, Street, Cellular, Zip: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  Country: Word; TimeZone: Byte; PublishEmail: Boolean;
  Age: Word; Gender: Byte; const HomePage: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  BirthYear: Word; BirthMonth, BirthDay: Byte;
  Language1, Language2, Language3: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  const About: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  AuthorizationRequired, WebAware: Boolean;
  var Seq, Seq2: Word);

procedure CreateCLI_METASETMORE(Pkt: PRawPkt; UIN: LongWord; Age: Word; Gender: Byte; HomePage: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  BirthYear: Word; BirthMonth, BirthDay, Lang1, Lang2, Lang3: Byte; var Seq, Seq2: Word);

procedure CreateCLI_METASETGENERAL(Pkt: PRawPkt; UIN: LongWord; const NickName, FirstName, LastName, Email, City, State, Phone, Fax, Street, Cellular, Zip: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Country: Word; TimeZone: Byte; PublishEmail: Boolean; var Seq, Seq2: Word);

procedure CreateCLI_METASETABOUT(Pkt: PRawPkt; UIN: LongWord; const About: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq, Seq2: Word);
procedure CreateCLI_SENDSMS(Pkt: PRawPkt; UIN: LongWord; const Destination, Text: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  CodePage: Word; const Time: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq, Seq2: Word);
procedure CreateCLI_SENDMSG_ADVANCED(Pkt: PRawPkt; Status: LongWord; ITime, IRandom, UIN: LongWord; const Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  RTFFormat: Boolean; var Seq: Word);
procedure CreateCLI_SENDMSG_FILEACK(Pkt: PRawPkt; Status: LongWord; FFSeq: Word; ITime, IRandom, UIN, FileSize: LongWord; const FileDesc, FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Port: Word; var Seq: Word);
procedure CreateCLI_SENDMSG_FILEDECLINE(Pkt: PRawPkt; FFSeq: Word; ITime, IRandom, UIN, FileSize: LongWord; const FileDesc, FileName, Reason: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Port: Word; var Seq: Word);
procedure CreateCLI_HELLO(Pkt: PRawPkt; var Seq: Word);
procedure CreateCLI_GOODBYE(Pkt: PRawPkt; var Seq: Word);
procedure CreateCLI_REGISTERUSER(Pkt: PRawPkt; const Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
...
procedure CreateCLI_SENDCONTACTS_REQ(Pkt: PRawPkt; Status: LongWord; ITime, IRandom, UIN: LongWord; Reason: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
procedure CreateCLI_UNREGUIN(Pkt: PRawPkt; UIN: LongWord; const Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq, Seq2: Word);
procedure CreateCLI_METASETPASS(Pkt: PRawPkt; UIN: LongWord; const Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Buffer: Pointer; BufLen: Word; var Seq, Seq2: Word);
...
procedure CreateCLI_REQAUTH(Pkt: PRawPkt; UIN: LongWord; Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word);
...
procedure CreateCLI_UPDATEGROUP(Pkt: PRawPkt; Name: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Tag: Word; IDs: TStringList; var Seq: Word);
procedure CreateCLI_UPDATEBUDDY(Pkt: PRawPkt; UIN, Name, SMSNumber: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Tag, ID: Word; BuddyType: Word; NotAuthorized: Boolean; var Seq: Word);
procedure CreateCLI_ADDBUDDY(Pkt: PRawPkt; UIN, Name, SMSNumber: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Tag, ID: Word; BuddyType: Word; NotAuthorized: Boolean; var Seq: Word);
procedure CreateCLI_DELETEBUDDY(Pkt: PRawPkt; UIN, Name, SMSNumber: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Tag, ID: Word; BuddyType: Word; NotAuthorized, IsGroup: Boolean; var Seq: Word);
procedure CreateCLI_SEND_YOU_WERE_ADDED(Pkt: PRawPkt; ITime, IRandom, UIN, FromUIN: LongWord; var Seq: Word);
procedure CreateCLI_TYPING_NOTIFICATION(Pkt: PRawPkt; UIN: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  NotificationType: Word; var Seq: Word); //ok3y
procedure CreateCLI_MD5AUTHREQ(Pkt: PRawPkt; UIN: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word); //ok3y

{Misc functions}
function SnacToStr(Family, SubType: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function SrvMetaToStr(V1, V2: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function PeerCmdToStr(Cmd: Byte): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function DumpPacket(Buffer: Pointer; BufLen: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function UnicodeCharCode2ANSIString (aCode: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  // yegor
function RTF2Plain (const aSource: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  // yegor
function StatusToInt(Value: LongWord): LongWord;   // added by eraser 30.6.03
function StatusToStr(Value: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};      // changed by eraser 30.6.03
function StatusToStr_rus(Value: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  //added by Volkov Ioann
function CountryToStr(Value: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function LanguageToStr(Value: Byte): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function OccupationToStr(Value: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function InterestToStr(Value: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function PastToStr(Value: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function AffiliationToStr(Value: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function LoadPacketRaw(Pkt: PRawPkt; const FName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
function LoadPacket(Pkt: PRawPkt; const FName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Flap: TFlapHdr; var Snac: TSnacHdr): Boolean;
function FileExists(const FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
function FileSize(const FName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): LongWord;
procedure LogText(const FName, Text: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure ShowMessage(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); overload;
procedure ShowMessage(Value: LongInt); overload;
function ExtractName(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function ExtractValue(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function UTF8ToStr(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function UTF8ToStrSmart(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function MyUTF8Decode(Value : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function UCS2BEToStr (Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  // yegor
function IsStringProbablyUCS2BE (aString: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean; // yegor
function StrToHexStr(AStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function HexStrToStr(AStr: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};

function GetXMLEntry(const Tag, Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};

{SMS functions}
function StrToUTF8(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function STime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function GetSMSTime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
function CreatePEER_MSG(Pkt: PRawPkt; const Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  RTFFormat: Boolean; var Seq: Word): Word;
...
procedure CreatePEER_AUTOMSG_ACK(Pkt: PRawPkt; Answer: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Status, Seq: Word);
function CreatePEER_CONTACTS(Pkt: PRawPkt; Contacts: TStringList; var Seq: Word): Word;
function CreatePEER_CONTACTREQ(Pkt: PRawPkt; const Reason: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  var Seq: Word): Word;
function CreatePEER_FILEINIT(Pkt: PRawPkt; Response: Boolean; FileDescription, FileName: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Port: Word; FileLength: LongWord; var Seq: Word; Reason: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Accept: Boolean): Word;
procedure CreatePEER_FILEINITACK(Pkt: PRawPkt; Speed: LongWord; Nick: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure CreatePEER_FILE_INIT(Pkt: PRawPkt; Count, Bytes, Speed: LongWord; Nick:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF});
procedure CreatePEER_FILE_INIT2(Pkt: PRawPkt; Count, Bytes, Speed: LongWord);
procedure CreatePEER_FILE_START(Pkt: PRawPkt; FileName:{$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  FileSize, Speed:LongWord);
Procedure CreatePEER_FILE_DATA(Pkt: PRawPkt; Buffer:Pointer; BufLen:Integer);


function EncodeBase64(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function DecodeBase64(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function CreateHTTP_Header(const Method, URL, Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  DataLen: LongWord; Auth: Boolean; User, Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
function CreateHTTP_INIT(Auth: Boolean; User, Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function CreateHTTP_RECV(Host, SID: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Auth: Boolean; User, Password: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
procedure CreateHTTP_LOGIN(Pkt: PRawPkt; Host: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Port: Word);


function Decrypt99bPassword(UIN, CryptIV: LongWord; const HexPass: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
function DecryptMirandaPassword(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
Var
  ...
  CAPS : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
begin
  for i := 1 to Length(Pass) do
    Pass[i] := AnsiChar(Ord(Pass[i]) xor TXorData[i - 1]);
end;
...
begin
  ...
  while StrLen > 0 do
  begin
    Result := Result + {$IFDEF VER230}PAnsiChar(LongWord(Pkt) + Pkt^.Len)^{$ELSE}PChar(LongWord(Pkt) + Pkt^.Len)^{$ENDIF};
    Inc(Pkt^.Len);
    Dec(StrLen);
    if Pkt^.Len > 8100 then Exit;
  end;
...
var
  i, l: Word;
  FName, FUIN: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
  l := 0; FName := ''; FUIN := '';
...
var
  i, count: Word;
  S: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
  count := 0;
  S := '';
...
begin
  ...
  begin
    ...
    PktSnac(Pkt, $17, $02, 0, 0);                 //Type x17/x02, ICQ_SIGNON_LOGIN_REQUEST
  end
  else
  begin
  ...
  if Secure then
  begin
    PktTlv(Pkt, $0025, Password);
    PktInt(Pkt, $004C0000, 4);
  end
  else
  begin
  ...
  PktTLV(Pkt, 3, 'ICQ Client');

  PktTLV(Pkt, $16, 2, $010A);  //Client ID
  PktTLV(Pkt, $17, 2, $0006);  //Major version
  PktTLV(Pkt, $18, 2, $0005);  //Minor version
  PktTLV(Pkt, $19, 2, $0000);  //Point version
  PktTLV(Pkt, $1A, 2, $0068);  //Build version
  PktTLV(Pkt, $14, 4, $00000753); //Dist Channel
  PktTLV(Pkt, $0F, 2, $656E);  //Country
  PktTLV(Pkt, $0E, 2, $7573);  //Language
  //PktTLV(Pkt, $94, 1, $00);
...
begin
  ...
  PktInt(Pkt, $00220001, 4);                    //Family x22 is Version x01 - added
  PktInt(Pkt, $00010004, 4);                    //Family x01 is Version x04 not x03 - modified
  PktInt(Pkt, $00130004, 4);                    //Family x13 at Version x04 not x02 - modified
...
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
  PktSnac(Pkt, $01, $06, $06, 0);               //Snac: Type x01/x06, ID x0000, Flags 0
  PktFinal(Pkt);                                //Finalize packet
end;
...
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
  PktSnac(Pkt, $01, $08, $08, 0);                 //Type x01/x08, ID x0000, Flags 0
...
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
  PktSnac(Pkt, $01, $0E, $0E, 0);                 //Snac: Type x01/x0E, ID x0000, Flags 0
  PktFinal(Pkt);                                //Finalize packet
end;

//It was written REQUNKNOWN before Volkov Ioann made changes
procedure CreateCLI_REQLISTS(Pkt: PRawPkt; var Seq: Word);
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
  PktSnac(Pkt, $13, $02, $02, 0);               //Snac: Type x13/x02, ID x0002, Flags 0
  PktInt(Pkt, $000B, 2);
  PktInt(Pkt, $0002000F, 4);
  PktFinal(Pkt);                                //Finalize packet
end;
...
procedure CreateCLI_SETUSERINFO(Pkt: PRawPkt; XStatNum : Byte; var Seq: Word);
begin
  if (XStatNum > 0) and (XStatNum <= XStatusesCount) then  //if XStatus is set, we add it to capabilities
    CAPS := CAPS + CapXStatus[XStatNum];
  PktInit(Pkt, 2, Seq);                         //Channel 2
  PktSnac(Pkt, $02, $04, 0, 0);                 //Snac: Type x02/x04, ID x0000, Flags 0
  PktTLV(Pkt, 5, Length(CAPS), @CAPS[1]);       //Client's capabilities
  PktFinal(Pkt);                                //Finalize packet
  if (XStatNum > 0) and (XStatNum <= XStatusesCount) then  //if XStatus was set, we delete it from capabilities
    CAPS := Copy(CAPS, 1, Length(CAPS) - 16);   //if XStatus was set, we delete it from capabilities
end;


//Volkov Ioann added 3 procedures for NewXStatuses, such as in ICQ6
function GetNewXStatus(st: T_NewXStatus): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  i: Word;
begin
...
begin
  ...
  PktInt(@lpkt, $000E, 2); //Type
  PktInt(@lpkt, $00, 1); //Flag
  PktLStr(@lpkt, 'icqmood' + IntToStr(MoodNum)); //Data
...
begin
  ...
  PktInt(Pkt, AChannel, 2);                        //chanel to setup
  {0x0B client supports typing notifications}
  if IsMtnEnabled  and (AChannel = 1)
    then PktInt(Pkt, $0000000B, 4)
    else PktInt(Pkt, $00000003, 4);

  PktInt(Pkt, $1F40, 2);                        //8000, by 0k3y: Max message snac size
  PktInt(Pkt, $03E7, 2);                        //999, by 0k3y: Max sender warning level
  PktInt(Pkt, $03E7, 2);                        //999, by 0k3y: Max receiver warning level
  PktInt(Pkt, 0, 2);                            //Minimum message interval
  PktInt(Pkt, 0, 2);                            //0, Unknown
  PktFinal(Pkt);                                //Finalize packet
                              //Finalize packet}
end;
...
begin
  ...
  PktInt(@lpkt, $00000000, 4);                  //Скроем нашу куку
  PktInt(@lpkt, $00000000, 4);                  //0, Web front port
  PktInt(@lpkt, $00000003, 4);                  //Client features: 3
  ...
  PktInt(@lpkt, InfoTime, 4);                  //Info time (update)
  PktInt(@lpkt, ExtInfoTime, 4);               //Ext info time (update)
  PktInt(@lpkt, ExtStatusTime, 4);             //Ext status time (update)

  PktInt(@lpkt, $0000, 2);                      //0, Unknown (Dummy?)
  PktTLV(Pkt, $0C, lpkt.len, @lpkt.Data);       //TLV(0C)

  //PktTLV(Pkt, $1D, #$00#$0E#$00#$00#$00#$02#$00#$00); //Empty NewXStatus
  PktTLV(Pkt, $1F, #$00#$00);                   //Unknown - Something as in ICQ6
...
const
  buf: array[0..87] of Byte = (
    $00, $22, $00, $01, $01, $10, $16, $4F, //Family x22
    $00, $01, $00, $04, $01, $10, $16, $4F,
    $00, $13, $00, $04, $01, $10, $16, $4F,
    $00, $02, $00, $01, $01, $01, $16, $4F,
...
procedure CreateCLI_SERVICExREQ(Pkt: PRawPkt; var Seq: Word);
begin
  PktInit(Pkt, 2, Seq);
  PktSnac(Pkt, $01, $04, $00, 0);
  PktInt(Pkt, $0010, 2); //requested service (family) id
  PktFinal(Pkt);
end;
...
begin
  ...
  PktSnac(Pkt, $10, $06, $06, $00); //SNAC(10,06) 	CLI_ICQ_ICON_REQUEST
  PktLStr(Pkt, UIN);                //UIN
  PktInt(Pkt, $01, 1);              //unknown (command ?)
  PktInt(Pkt, AvatarId, 2);
  PktInt(Pkt, AvatarFlags, 1);
  PktLStr(Pkt, AvatarHash);
  PktFinal(Pkt);
end;
...
begin
 ...
 // PktTLV(Pkt, 3, 0, 0);                         //Request server ack TLV
  PktTLV(Pkt, 6, 0, 0);                         //Store on server when user offline
  PktFinal(Pkt);                                //Finalize packet
end;
...
procedure CreateCLI_SEARCHWP(Pkt: PRawPkt; UIN: LongWord; First, Last, Nick, Email: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  MinAge, MaxAge: Word;
  Gender: Byte; Language: Byte; City: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Country: Word; Company, Department, Position: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  Occupation: Byte; Organization: Word; OrganKeyWords: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Affiliation: Word; AffiKeyWords,
  KeyWord: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Online: Byte; var Seq, Seq2: Word);
var
  lpkt: TRawPkt;
begin
...
begin
  ...
  PktInt(@lpkt, $3A0C, 2);      //CLI_SET_FULLINFO
  PktLTlv(@lpkt, TLV_NICKNAME, NickName);
  PktLTlv(@lpkt, TLV_FIRSTNAME,  FirstName);
  PktLTlv(@lpkt, TLV_LASTNAME,  LastName);
  {email tlv}
  PktLBTlv(@lpkt, TLV_EMAIL, Email, Ord(not PublishEmail));

  PktLTlv(@lpkt, TLV_CITY,  City);
  PktLTlv(@lpkt, TLV_STREET,  Street);

  PktLTlv(@lpkt, TLV_STATE,  State);

  PktLTlv(@lpkt, TLV_PHONE,  Phone);
  PktLTlv(@lpkt, TLV_FAX,  Fax);
  PktLTlv(@lpkt, TLV_MOBILE,  Cellular);

  PktLTlv(@lpkt, TLV_ZIPCODE,  Zip);
  PktLTlv(@lpkt, TLV_COUNTRY,  2,  Country);
  PktLTlv(@lpkt, TLV_TIMEZONE, 1,  TimeZone);

  PktLTlv(@lpkt, TLV_AGE, 2, Age);
  {make a birth tlv}
  if (BirthYear <> 0) and (BirthMonth <> 0) and (BirthDay <> 0) then
  begin
    PktLInt(@lpkt, TLV_BIRTH,2);
    PktLInt(@lpkt, 6,2); //size of tlv
    PktLInt(@lpkt, BirthYear, 2);
    PktLInt(@lpkt, BirthMonth, 2);
    PktLInt(@lpkt, BirthDay, 2);
  end;
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, StrToLanguageI(Language1));
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, StrToLanguageI(Language2));
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, StrToLanguageI(Language3));

  PktLTlv(@lpkt, TLV_URL, HomePage);
  PktLTlv(@lpkt, TLV_GENDER, 1, Gender);

  PktLTlv(@lpkt, TLV_ABOUT,  About);
  PktLTlv(@lpkt, TLV_AUTH, 1, Ord(not AuthorizationRequired));
  PktLTlv(@lpkt, TLV_WEBAWARE, 1, Ord(not WebAware));
...
begin
  ...
  {languages tlv}
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, Lang1);
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, Lang2);
  PktLTlv(@lpkt, TLV_LANGUAGE, 2, Lang3);
...
begin
  PktInitRaw(@lpkt);
  PktInt(@lpkt, $3A0C, 2); //CLI_SET_FULLINFO
  PktLTlv(@lpkt, TLV_ABOUT,  About);   //The about information string to set.
  CreateCLI_TOICQSRV(Pkt, UIN, $07D0, @lpkt, lpkt.Len, Seq, Seq2); //Incapsulate in CLI_TOICQSRV
end;
...
procedure CreateCLI_SENDADVMSG_CUSTOM(Pkt: PRawPkt; Status: LongWord; FFSeq: Word;
  ...
  const Msg: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Data: Pointer; DataLen: LongWord; RTFFormat: Boolean;
...
procedure CreateCLI_GOODBYE_AS_HELLO(Pkt: PRawPkt; var Seq: Word);
begin
  PktInit(Pkt, 1, Seq);                         //Channel 2
  PktFinal(Pkt);                                //Finalize packet
end;
...
procedure __CreateCLI_UPDATEBUDDY(Pkt: PRawPkt; A: Byte; UIN, Name, SMSNumber: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  Tag, ID: Word; BuddyType: Word; NotAuthorized, IsGroup: Boolean; var Seq: Word);
var
  TLVs: TRawPkt;
begin
  PktInit(Pkt, 2, Seq);                         //Channel 2
  if ID = 0
    {group id as Idreq}
    then PktSnac(Pkt, $13, A, Tag, 0)
    {user id as Idreq}
    else PktSnac(Pkt, $13, A, ID, 0);
...
begin
  PktInit(Pkt, 2, Seq);               //channel 2
  PktSnac(Pkt, $04, $14, 0, 0);       //Snac: Type x04/x14, ID x0000, Flags 0
  PktInt(Pkt, 0, 4);                  //notification-id cookie
  PktInt(Pkt, 0, 4);                  //notification-id cookie
  PktInt(Pkt, $01, 2);                //notification channel
  PktLStr(Pkt, UIN);                  //uin
  PktInt(Pkt, NotificationType, 2);   //Notification Type
  PktFinal(Pkt);
end;
...
begin
  PktInit(Pkt, 2, Seq);
  PktSnac(Pkt, $17, $06, 0, 0);       //Snac: Type x17/x06 md5-authkey request
  PktTlv(Pkt, $0001, UIN);
  PktInt(Pkt, $004B0000, 4);
  PktFinal(Pkt);
end;
...
begin
  ...
  else if (Family = $04) and (SubType = $14) then
    Result := 'TYPING_NOTIFICATION'      // Send and receive SNACs has the same type=0x14 : ok3y
  {SRV}
    ...
    begin
      ...
      begin
        if (PByte(LongWord(Buffer) + n - 1)^ < $20) or (PByte(LongWord(Buffer) + n - 1)^ > $7F) then
          ...
          S := S + {$IFDEF VER230}PAnsiChar(Buffer){$ELSE}PChar(Buffer){$ENDIF}[n - 1];
      end;
      S := S + #13#10;
    end;
  end;
  ...
  begin
    ...
    begin
      if (PByte(LongWord(Buffer) + BufLen - i)^ < $20) or (PByte(LongWord(Buffer) + BufLen - i)^ > $7F) then
        ...
        S := S + {$IFDEF VER230}PAnsiChar(Buffer){$ELSE}PChar(Buffer){$ENDIF}[BufLen - i];
    end;
  end;
  Result := S;
end;

{Convert Unicode char code into string in local ANSI encoding}
function UnicodeCharCode2ANSIString (aCode: Word): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
   WideChar: array [0..1] of Word;
begin
     WideChar [0]:=aCode;
     WideChar [1]:=0;
     UnicodeCharCode2ANSIString:=WideCharToString (@WideChar) [1];
end;

{Convert RTF enabled text to plain.}
function RTF2Plain (const aSource: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
   Source: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
   NChar: Integer;

function ProcessGroupRecursevly: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
var
   Control, NumericValue, TextValue: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
     Result:='';
     Inc (NChar);
                                                  ...
                                                  if (Control='line') or (Control='par')
                                                     then Result:=Result+#$0D#$0A
                                                             ...
                                                             then Result:=Result+#$09
                                                                     ...
                                                                     then Result:=Result+UnicodeCharCode2ANSIString (StrToInt (NumericValue))
...
function StatusToStr(Value: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
...
function StatusToStr_rus(Value: LongWord): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};  //added by Volkov Ioann
var
  Rs : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
  if (Value and S_DND) = S_DND then
    Rs := 'Не беспокоить'
  else if (Value and S_OCCUPIED) = S_OCCUPIED then
    ...
    Rs := 'Готов поболтать'
  else if (Value and S_HOME) = S_HOME then
    Rs := 'Дома'
  else if (Value and S_DEPRESSION) = S_DEPRESSION then
    Rs := 'Депрессия'
  else if (Value and S_EATING) = S_EATING then
    Rs := 'Кушаю'
  else if (Value and S_ANGRY) = S_ANGRY then
    Rs := 'Злой'
  else if (Value and S_WORK) = S_WORK then
    Rs := 'На работе'
  else Rs := 'Онлайн';

  if Value = S_OFFLINE then
    Rs := 'Оффлайн'
  else if Value = S_QIP_HOME then
    Rs := 'Дома'
  else if Value = S_QIP_ANGRY then
    Rs := 'Злой'
  else if Value = S_QIP_DEPRESSION then
    Rs := 'Депрессия'
  else if Value = S_QIP_EATING then
    Rs := 'Кушаю'
  else if Value = S_QIP_WORK then
    Rs := 'На работе'
  else if Value = S_QIP_NA then
    Rs := 'Недоступен'
  else if Value = S_QIP_OCCUPIED then
    Rs := 'Занят'
  else if Value = S_QIP_DND then
    Rs := 'Не беспокоить'
  else if Value = S_QIP_INVISIBLE then
    Rs := 'Невидимый';
...
begin
  ...
  {$IFDEF VER230}
  FileHandle := CreateFileA(PAnsiChar(FName), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE, nil, OPEN_ALWAYS, 0, 0);
  {$ELSE}
...
procedure ShowMessage(const Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}); overload;
begin
  MessageBoxA(0, {$IFDEF VER230}PAnsiChar(Value){$ELSE}PChar(Value){$ENDIF}, 'Message', 0);
end;

procedure ShowMessage(Value: LongInt); overload;
begin
  MessageBoxA(0, {$IFDEF VER230}PAnsiChar(IntToStr(Value)){$ELSE}PChar(IntToStr(Value)){$ENDIF}, 'Message', 0);
end;
...
function UTF8ToStrSmart(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  Digit: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  i: Word;
  HByte: Byte;
  Len: Byte;
begin
...
//Added by Volkov Ioann to recognize russian without utf8
function MyUTF8Decode(Value : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}) : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
  Rs : {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
begin
...
{Convert string from UCS2BE format} // yegor
function UCS2BEToStr (Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
var
   NWord: LongInt;
begin
     Result:='';
     for NWord:=1 to Length (Value) shr 1
         do Result:=Result+UnicodeCharCode2AnsiString ((Ord (Value [(NWord shl 1)-1]) shl 8)+Ord (Value [NWord shl 1]));
end;

{Checks if string has UCS2BE format} // yegor
function IsStringProbablyUCS2BE (aString: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): Boolean;
var
   Buffer: array of ANSIChar;
   NChar: LongInt;
begin
     if Length (aString) and 1=0
        then begin
             SetLength (Buffer, Length (aString));
             for NChar:=1 to Length (aString)
                 do Buffer [(NChar-1) xor 1]:=aString [NChar];
             Result:=IsTextUnicode (Buffer, Length (aString), nil);
        end
        else Result:=false;
end;
...
begin
  ...
  begin
    Result := Result + Char( HexToInt(copy(AStr,i,2) ) );
    Inc(I,2);
  end;
...
begin
  ...
  FreeMem(buffer, BufLen);
  Result := {$IFDEF VER230}PAnsiChar(lpBuf){$ELSE}PChar(lpBuf){$ENDIF};
...
begin
  ...
  recv_bytes := GetTimeFormatA(LANG_ENGLISH, TIME_FORCE24HOURFORMAT,
    @SysTime, {$IFDEF VER230}PAnsiChar('HH:mm:ss'){$ELSE}PChar('HH:mm:ss'){$ENDIF}, @buf, SizeOf(buf));
...
function GetSMSTime: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  ...
  function SDate: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  var
    buf: array[0..15] of Char;
    recv_bytes: Integer;
    SysTime: TSystemTime;
...
const
  client_check_data: {$IFDEF VER230}PAnsiChar{$ELSE}PChar{$ENDIF} =
...
const
  b64alphabet: {$IFDEF VER230}PAnsiChar{$ELSE}PChar{$ENDIF} = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';

function EncodeBase64(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
const
  pad: {$IFDEF VER230}PAnsiChar{$ELSE}PChar{$ENDIF} = '====';

  function EncodeChunk(const Chunk: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  var
    W: LongWord;
    i, n: Byte;
  begin
...
function DecodeBase64(Value: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  function DecodeChunk(const Chunk: {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF}): {$IFDEF VER230}RawByteString{$ELSE}string{$ENDIF};
  var
    W: LongWord;
    i: Byte;
  begin
...
function UnixDateTimeToDateTime (aUnixTime: LongInt): TDateTime;
begin
    Result:=aUnixTime/SecPerDay+Offset1970;
end;
...
Begin
  fTrigger.Terminate;
  if not fTrigger.Suspended
     then fTrigger.WaitFor
     else fTrigger.Free;
  inherited;
End;
}

//Rewritten by Volkov Ioann
destructor TThreadTimer.Destroy;
Begin
  fTrigger.Terminate;
...
begin
  CAPS := caps_aim_interoperate + caps_avatar +
          ...
          caps_xtraz;  //default capabilities
  InfoTime := $00000000;
  ExtInfoTime := $00000000;
  ExtStatusTime := $00000000;
end.
